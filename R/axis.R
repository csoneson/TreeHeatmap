...
xlim2 <- function(ggheat, limits = NULL) {
    axis_align(ggheat = ggheat, limits = limits, axis = 'x')
}

##' @rdname align_axis
##' @title ylim2
##' @export
ylim2 <- function(ggheat, limits = NULL) {
    axis_align(ggheat = ggheat, limits = limits, axis = 'y')
}

axis_align <- function(ggheat, limits = NULL, axis) {
    gg <- ggheat$plotlist[[1]]
...
ggplot_add.axisAlign <- function(object, plot, object_name) {
    ...
    if (is.numeric(limits)) {
        lim_x <- scale_x_continuous(limits=limits, expand=c(0,0),
                                    breaks = anchor$col_anchor$x,
                                    labels = anchor$col_anchor$colLab)
        lim_y <- scale_y_continuous(limits = limits, expand = c(0, 0),
                                    breaks = anchor$row_anchor$y,
                                    labels = anchor$row_anchor$rowLab)
    } else {
